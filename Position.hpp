 ...
 */
        ...
        public:
             ...
             *
             * @param col: integer from 0 that marks the column in which we unodo the last move
             *
             * */
            void undo(int col) {
                position_t move = ((mask + bottom_mask_col(col)) & (column_mask(col)<<1))>>1;
                moves--;
                mask ^= move;
                current_position ^= mask;
            }
            ...
            /**
             * Outputs the state of the board as a text where:
             * - . are empty fields
             * - X positions taken by player 1
             * - O positions taken by player 2
             *
             * */
            void print_board() {
                std::string b1 = std::bitset<WIDTH * (HEIGHT + 1)>(current_position).to_string();
                std::string b2 = std::bitset<WIDTH * (HEIGHT + 1)>(mask ^ current_position).to_string();
                ...
                b.resize(HEIGHT + 1, std::vector<char>(WIDTH, '.'));

                for (int i = 0; i < b1.length(); i++) {
                    if (b2[i] == '1')
                        b[i % 7][WIDTH - 1 - i / 7] = c1;

                    if (b1[i] == '1')
                        b[i % 7][WIDTH - 1 - i / 7] = c2;
                }
            ...
            */
            uint64_t key3() const {
                ...
                return key_forward < key_reverse ? key_forward / 3 : key_reverse /
                                                                     3; // take the smallest key and divide per 3 as the last base3 digit is always 0
            }
        ...
        private:
            ...
            struct bottom {
                static constexpr position_t mask =
                        bottom<width - 1, height>::mask | position_t(1) << (width - 1) * (height + 1);
            };
            template<int height>
